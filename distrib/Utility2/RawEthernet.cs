...
namespace Utility2
{
	...
	public class RawEthernet
	{
		...
		public void SendPacket(string str)
		{
			...
			if (!rawether.BindAdapter(aiAdapters[0].AdapterID))
			{
				// alert user and quit
			...
			else
			{
				...
				Console.WriteLine("\nAdapter " + aiAdapters[0].AdapterID +
					" is ready for writing...\n");
			}

			// ok, now we are ready to write a raw packet on the adapter
			// create a generic raw packet (must be at least 16 bytes long)
            /*byte[] packet = new byte[] {0x00,0x0B,0xAB,0x0A,0xED,0x25,  //destination mac  000BAB0AED25
										0x00,0x50,0x04,0x4A,0xB8,0x32,  //source mac 0050044AB832
										0x08,0x00,					    //protocol
										0x01,0x02,0x03,0x04,0x05,0x06}; //generic data*/
			
			// write the packet to the device a few times
			//rawether.DoWrite(data);
            string hex = "0x00,0x1C,0xC0,0x9C,0x88,0x28,0x00,0x50,0x04,0x4A,0xB8,0x54,0x08,0x00,0x45,0x00,0x00,0x4E,0x00,0x7A,0x00,0x00,0x80,0x11,0xB5,0x62,0xC0,0xA8,0x01,0x7B,0xC0,0xA8,0x01,0x82,0x00,0x89,0x00,0x89,0x00,0x3A,0x77,0x07,0x80,0x1F,0x01,0x10,0x00,0x01,0x00,0x00,0x00,0x00,0x00,0x00,0x20,0x45,0x43,0x45,0x42,0x45,0x44,0x45,0x4C,0x46,0x46,0x46,0x41,0x43,0x4F,0x45,0x42,0x46,0x47,0x45,0x48,0x43,0x4F,0x45,0x44,0x46,0x4B,0x43,0x41,0x43,0x41,0x41,0x41,0x00,0x00,0x20,0x00,0x01";
            char[] c = new char[] {','};
            string[] del = str.Split(c);
            //hex = hex.Replace('-', ' ');
            //hex = hex.Trim();

            byte[] packet = new byte[del.Length] ;/*{ 0x00,0x1C,0xC0,0x9C,0x88,0x28, 
            ...
            {
                packet[i] = Convert.ToByte(del[i],16);
            }
